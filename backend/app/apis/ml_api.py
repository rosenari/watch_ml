...
async def create_ml_model(request: ModelCreateRequest, ml_service: MlService = Depends(get_ml_service)):
    try:
        ...
        create_model_task.delay(request.model_name, request.model_type, version, request.zip_files)
...
async def deploy_ml_model(request: ModelCreateRequest):
    try:
        deploy_model_task.delay(request.model_name, request.model_type)
...
async def deploy_ml_model(request: ModelCreateRequest, ml_service: MlService = Depends(get_ml_service)):
    try:
